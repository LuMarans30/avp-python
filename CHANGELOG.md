...
- **Cross-family projection on `LlamaCppConnector`** — `_project_rosetta` now auto-selects the projection from the two GGUF vocabularies: vocabulary-mediated when the token maps match, and `vocab_overlap_projection` (shared-token indices) when they differ (e.g. BPE ↔ SentencePiece). Adds `extract_gguf_vocab()` and `compute_vocab_overlap_from_dicts()`. Cross-model `generate()` no longer tries to reuse the source's live `llama_context`.
...
- **Cached GGUF projection maps + `linear` method** — cross-model projection artifacts (shared-token embedding rows, target norm, fitted map) are built once per model pair and persisted under `$AVP_CACHE_DIR/gguf_maps`, turning the ~75 s dequantization into a one-time cost. New `--projection-method linear` fits a ridge-regression map over shared-token embeddings; on a held-out token split (Ling↔Gemma) it scores 0.52 mean cosine to the true target embedding vs 0.36 for `vocab_overlap`. Adds `avp.rosetta.gguf_map`.
- **Multi-state cross-model projection** — llama.cpp `think()` records per-step hidden states on `AVPContext.hidden_states`, and cross-model `generate()` projects the whole trajectory and injects it as a sequence rather than only the final vector. This improves alignment bandwidth but does not make verbatim recall reliable across families; cross-model still suits structured reasoning over memorization.
- **Cross-model transfer investigation** — design note (`docs/CROSS_MODEL_TRANSFER.md`) and an experiment harness (`experiments/`) recording the payload / injection / translation-fidelity findings, and a go/no-go gate for a learned cross-model translator.

### Fixed

- **Soft-prompt injection into scaled-embedding models** — `_generate_with_embedding` now applies the target model's input-embedding scale (Gemma/T5 use `sqrt(n_embd)`). llama.cpp applies this scale for token ids but not for embeddings injected via `batch.embd`, so injected soft prompts were mis-scaled and unreadable (e.g. Gemma could not read even its own token embeddings).
